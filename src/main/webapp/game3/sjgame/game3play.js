class game3play extends Phaser.Scene {
    constructor() { super({ key: "game3play" }); 
	}
    ...
    preload() {
       const basePath = "/game3/sjgame/asset/";
    ...
    create() {
        ...
        this.physics.add.collider(this.player, this.boxGroup, () => {
			$.ajax({
				...
				type:"POST",
				data:{
					game_id: 3,                  // 게임 번호 (예: 3번 게임)
					user_name: window.userName,  // JSP에서 세션 값 전달
					score: this.elapsed.toFixed(0)           // 넘겨받은 점수
				},
			});
            this.scene.start("game3over", { finalTime: this.elapsed.toFixed(0) });
        });
